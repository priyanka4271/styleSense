...
const C = { dark:'#1C1A18', mid:'#6B6560', accent:'#C4A882', cream2:'#EEE8DF', white:'#FDFCFB' };
...
export default function Step1_PersonalInfo({ onNext, data }) {
  ...
  const valid = form.name && form.age && form.gender;
  ...
  return (
    <div>
          ...
          <div style={{ display:'flex', gap:'12px' }}>
            {['Women', 'Men', 'Non-binary'].map(g => (
              <button key={g} onClick={() => set('gender', g)} style={{
                flex:1, padding:'12px', border:`0.5px solid ${form.gender===g ? C.dark : '#D9D2C8'}`,
      ...
      <button
        onClick={() => onNext(form)} disabled={!valid}
        style={{
